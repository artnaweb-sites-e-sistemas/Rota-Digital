...
import { getAdminRevenueSeries } from "@/lib/admin-revenue-series";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const body = await getAdminRevenueSeries(gate.ctx.db, q);
    return NextResponse.json(body);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Erro ao obter série de receita real.";
    console.error("[admin-revenue-series]", e);
