...
interface HyperspeedOptions {
  ...
  colors: Colors;
  isHyper?: boolean;
  /** Multiplicador do tempo da cena (1 = padrão; menor = mais lento). */
  animationTimeScale: number;
  /** Limiar do bloom (maior = menos brilho “neon”). */
  bloomLuminanceThreshold?: number;
  /** Transparência dos trilhos de luz (0–1). */
  carLightsOpacity?: number;
  /** Transparência dos postes de luz laterais (0–1). */
  lightSticksOpacity?: number;
}
...
/** Preset alinhado à identidade Rota Digital (ouro oliva, creme, asfalto quente). Referência estável para a hero. */
export const ROTA_HERO_HYPERSPEED_OPTIONS: Partial<HyperspeedOptions> = {
  ...
  fovSpeedUp: 150,
  speedUp: 1.2,
  animationTimeScale: 0.46,
  bloomLuminanceThreshold: 0.52,
  carLightsOpacity: 0.26,
  lightSticksOpacity: 0.18,
  carLightsFade: 0.55,
  ...
  movingAwaySpeed: [17, 25],
  movingCloserSpeed: [-36, -50],
  carLightsLength: [400 * 0.04, 400 * 0.16],
  carLightsRadius: [0.05, 0.12],
  carWidthPercentage: [0.28, 0.48],
  carShiftX: [-0.35, 0.35],
  carFloorSeparation: [0.05, 2.5],
  colors: {
    roadColor: 0x12100e,
    islandColor: 0x161411,
    background: 0x090807,
    shoulderLines: 0x2a2620,
    brokenLines: 0x2a2620,
    leftCars: [0x8f7348, 0x6e623e, 0x9a7f58],
    rightCars: [0xc9bc9f, 0xb5a88a, 0x9e9178],
    sticks: 0x9a8a68,
  },
};

/**
 * Aproximação do `--background` em modo claro (`oklch(0.985 0.004 85)` em globals.css).
 * Usada para “apagar” o asfalto e a ilha no WebGL, mantendo só trilhos de luz.
 */
const ROTA_HERO_LIGHT_PAPER = 0xfaf9f7;

/** Preset da hero conforme o tema resolvido (estrada escura só no dark). */
export function getRotaHeroHyperspeedOptions(resolvedTheme: string | undefined): Partial<HyperspeedOptions> {
  if (resolvedTheme !== 'light') {
    return ROTA_HERO_HYPERSPEED_OPTIONS;
  }
  const paper = ROTA_HERO_LIGHT_PAPER;
  return {
    ...ROTA_HERO_HYPERSPEED_OPTIONS,
    bloomLuminanceThreshold: 0.5,
    carLightsOpacity: 0.52,
    lightSticksOpacity: 0.4,
    carLightsFade: 0.48,
    colors: {
      ...ROTA_HERO_HYPERSPEED_OPTIONS.colors!,
      roadColor: paper,
      islandColor: paper,
      background: paper,
      shoulderLines: paper,
      brokenLines: paper,
      leftCars: [0x7a5c32, 0x8e7d4d, 0x6b4f2a],
      rightCars: [0xc4b896, 0xa89472, 0x8e7d62],
      sticks: 0x8e7d4d,
    },
  };
}
      ...
      uniforms: Object.assign(
        {
          ...
          uCarLightsOpacity: { value: options.carLightsOpacity ?? 1 }
        },
...
const carLightsFragment = `
  ...
  varying vec2 vUv; 
  uniform vec2 uFade;
  uniform float uCarLightsOpacity;
  void main() {
    vec3 color = vec3(vColor);
    float alpha = smoothstep(uFade.x, uFade.y, vUv.x) * uCarLightsOpacity;
      ...
      uniforms: Object.assign(
        {
          uTravelLength: { value: options.length },
          uTime: { value: 0 },
          uLightSticksOpacity: { value: options.lightSticksOpacity ?? 1 }
        },
        this.webgl.fogUniforms,
...
const sideSticksFragment = `
  ...
  ${THREE.ShaderChunk['fog_pars_fragment']}
  varying vec3 vColor;
  uniform float uLightSticksOpacity;
  void main(){
    vec3 color = vec3(vColor);
    gl_FragColor = vec4(color, uLightSticksOpacity);
    ${THREE.ShaderChunk['fog_fragment']}
  }
`;
...
class App {
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(
      this.camera,
      new BloomEffect({
        luminanceThreshold: this.options.bloomLuminanceThreshold ?? 0.2,
  ...
  update(delta: number) {
    ...
    const time = (this.clock.elapsedTime + this.timeOffset) * this.options.animationTimeScale;
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS, className }) => {
  ...
  return <div id="lights" className={cn('h-full w-full', className)} ref={hyperspeed} />;
};
