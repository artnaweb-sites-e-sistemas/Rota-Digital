...
export function PublicThemeToggle({ className }: PublicThemeToggleProps) {
  ...
  if (!mounted) {
    return (
      <span
        className={cn(
          "inline-flex h-10 w-10 shrink-0 rounded-md border border-transparent sm:h-9 sm:w-9",
          className,
        )}
        aria-hidden
      />
  ...
  return (
    ...
    >
      {isDark ? (
        <Sun className="size-[1.35rem] text-amber-500 dark:text-amber-400 sm:size-5" aria-hidden />
      ) : (
        <Moon className="size-[1.35rem] text-indigo-600 dark:text-indigo-400 sm:size-5" aria-hidden />
      )}
    </button>
  );
}
